...
function customAssign(...objects) {
  ...
  const allObjects = Object.assign({}, ...objects);
  for (const key in allObjects)
    if (typeof allObjects[key] === "number") {
      allObjects[key] = String(allObjects[key]);
    }
  return allObjects;
...
function customObjectFromEntries(entries) {
  ...
  const makeMap = entries.map(([key, value]) => {
    const aboutKey = isNaN(key) ? key : key.toString();

    const aboutValue = typeof value === "number" ? value.toString() : value;

    return [aboutKey, aboutValue];
  });

  const result = Object.fromEntries(makeMap);
...
function checkProperty(obj, prop) {
  ...
  const getOwnProperty = obj.hasOwnProperty(prop);
...
function filterObjectsByKey(arr, key) {
  ...
  const getFilter = arr.filter((obj) => obj.hasOwnProperty(key));
  return getFilter;
...
function checkOwnership(obj1, obj2, key) {
  const firstKey = obj1.hasOwnProperty(key);
  const secondKey = obj2.hasOwnProperty(key);
...
function getObjectValuesSum(obj) {
  ...
  const sum = getValues.reduce((accum, value) => accum + value, 0);
  return sum;
